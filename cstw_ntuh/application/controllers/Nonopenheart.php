...
class Nonopenheart extends CI_Controller {
    ...
    function savenonopenheart(){
      ...
      $month=$this->input->post('qmonth');
        $query = $this->Nonopenheart_Model->query_nonopenheart($year,$month);
        if($query->num_rows() ==1){
    ...
    function querynonopenheart(){
      ...
      function querynonopenheartlist(){
            ...
            $query = $this->Nonopenheart_Model->query_nonopenheartlist();
        if($query->num_rows() >0){
              $arr=array('status'=>'success','result'=>$query->result());
        } else {
             $arr=array('status'=>'fail','result'=>'');
       ...
       function queryVascular(){
            $this->load->model('Analysis_Model');
         $qYear=$this->input->post('qYear');
      $qMonth=$this->input->post('qMonth');
      $item=$this->input->post('myitem');
        $query = $this->Analysis_Model->query_executivesummaryVascular($qYear,$qMonth,$qYear,$qMonth,$item)->row()->num;;
        if(1==1){
              $arr=array('status'=>'success','result'=>$query);
         //$access_id=accessLog('R','NONEOPENHEART',$query->row()->nid,$this->session->userdata('userRealname').'查詢【 Non-Open Heart】(月份:'.$year.'/'.$month.')','S');
