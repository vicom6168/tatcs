    ...
    $this->load->library('session');
        ...
        $this->load->library('patientinformationClass');
               ...
               // $patientinformationClass = $this->PatientInformation_Model->viewRecord($patientID)->row();
                ...
                $patientinformationClass->Procedure_Others=$this->input->post('ProcedureOthers');
                        //20161201修改開始
                ...
                $patientinformationClass->ReOperation=$this->input->post('patientReoperation');
                ...
                if($surgeon4->num_rows() ==1){
                     ...
                     $patientinformationClass->patientSurgeon_associalid4=$surgeon4->row()->associateID;
                }
                        }
                        if($this->input->post('patientSurgeon5')!=""){
                        $surgeon5=$this->user_Model->queryUserbyRealname($this->input->post('patientSurgeon5'));
                if($surgeon5->num_rows() ==1){
                             $patientinformationClass->patientSurgeon_id5=$surgeon5->row()->userID;
                     $patientinformationClass->patientSurgeon_associalid5=$surgeon5->row()->associateID;
                }
                        }
                        //20161201修改結束
...
function printPatient($pid){
  ...
  function queryDiagnosis($d,$t){
       $this->load->model('PatientInformation_Model');
   
       $data['Diagnosis']=$this->PatientInformation_Model->selectDiagnosis($t);
       $data['w']= $d;
 ...
 function mailBody($pid)
    {
        ...
        $html.="<td>".str_replace('0000-00-00', '', $c->DischargeDate)."</td>";
        $html.="</tr>";
        $html.="<tr>";
