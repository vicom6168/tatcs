...
class PatientInformation_Model extends CI_Model {
    ...
    function query_uncompletelist($qField,$qOrder,$qCondition,$qstr,$from,$count){
             $sql= "SELECT *,";
             $sql.="if(patientChartNumber =''  or  patientName=''  or  patientGender=''  or patientBirthday='' or patientBirthday='0000-00-00'";
             $sql.=" or patientOpDate='' or patientOpDate='0000-00-00' or patientOpDate is null ";
             $sql.=" or AdmissionDate='' or AdmissionDate='0000-00-00' or AdmissionDate is null ";
             $sql.=" or DischargeDate='' or DischargeDate='0000-00-00' or DischargeDate is null ,'Y','N') as uncomplete_1, ";
             $sql.=" if(patientSurgeon=''  or patientSurgeon is null or diseaseType='' or diseaseType is null or  " ;
             $sql.= " ((Procedure_id1=''  or Procedure_id1 is null) and (ProcedureOthers=''  or ProcedureOthers is null) )  or " ;
        ...
        if($qstr!="" && $qstr!="999999999"){
          ...
          if($qCondition=="0" || $qCondition=="1"){
            ...
            $subsql.= " or  ((Procedure_id1=''  or Procedure_id1 is null) and (ProcedureOthers=''  or ProcedureOthers is null)  ) ";
...
function Save_patienthistory($patientinformationClass){
     ...
     function Update_patientfromUpload($UUID,$patientinformationClass){
       ...
       function insert_age($age,$ageType, $pID){
        ...
        function selectDiagnosis(){
           $sql= " select * from surgerydiagnosis order by  surgerydiagnosis.categoryorder, surgerydiagnosis.order ";
        ...
        function selectProcedure($t){
            if($t!="" && $t!="99"){
                $sql= " select * from  surgeryprocedure where cancertype='$t' order by  surgeryprocedure.order, surgeryprocedure.code ";
            }  else {
           $sql= " select * from surgeryprocedure order by  surgeryprocedure.order, surgeryprocedure.code ";
           }
        return $this->db->query($sql); 
       }
   ...
   function export_uploadpatientdo($d1,$d2,$h){
            $sql= "SELECT * FROM patientinformation t1 where  patientOpDate>='$d1' and patientOpDate<='$d2' ";
         if($h!='')
             $sql.= "  and patientHospital=?";
