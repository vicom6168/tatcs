...
class Contact_Model extends CI_Model {
    ...
    function update_contact($contactid,$contactClass){
          $otherdb = $this->load->database('ad', TRUE);
        $otherdb->where('contactid', $contactid);
        $otherdb->update('contact', $contactClass);
    }
   
      function delete_contact($nid){
           $otherdb = $this->load->database('ad', TRUE);
    $otherdb->where('contactid', $nid);
   $otherdb->delete('contact');
    }
