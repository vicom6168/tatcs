                           ...
                           echo $row->patientSurgeon;
                            ?></td>
                             <td style="padding : 2px 8px;line-height : 10px;"><img src="<?php echo $completeImage[$row->uncomplete];?>"  title="<?php echo $completeWords[$row->uncomplete];?>"  alt="<?php echo $completeWords[$row->uncomplete];?>"  width="16" height="16"></td>
                                <td style="padding : 2px 8px;line-height : 10px;">
                                    <button  class="blue medium" onclick="javascript: window.open('<?php echo base_url(); ?>patient/viewRecord/<?php echo $row->patientID;?>/divLVAD','new');"><span>查看</span></button>
...
<script>
    function qryPatient(){
        if($('#qryText').val()=="" && 1==2){
            ...
            window.location='<?php echo base_url(); ?>specialSheet/LVAD/'+$('#qryField').val()+'/'+$('#qryOrder').val()+'/'+$('#qryText').val();
        }
    }
</script>
