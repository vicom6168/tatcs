...
elseif($c->patientAgeUnit=="2"){
    ...
    echo   "Days";
                    ...
                    <div class="line button">
                              ...
                              <button type="button" id="sendFrom" class="blue medium" onclick="sentForm()"><span>存檔</span></button>
                               <?php if($c->patientOpDate=="" || $c->patientOpDate=="0000-00-00") { ?>
                                    <button  class="grey medium" id="transButton<?php echo $c->patientID;?>" 
                                    onmouseover="$(this).notify('請先填寫OP Date並存檔後, \n\n 才能匯入病患資料庫','error');"
                                    onmouseout="$(this).notify('');"
                                    onclick="javascript:return false;" ><span>匯入病患資料</span></button>
                                    <?php } else { ?>
                                        
                            <button  class="orange medium" id="transButton" onclick="javascript:if(confirm('資料轉入後會再術前評估中資料刪除?')){ evaluationTrans('<?php echo $c->patientID;?>');return false;}" ><span>匯入病患資料</span></button>
                            <?php }  ?>
