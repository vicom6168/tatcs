...
public function editcontact($id){
        $query = $this->Contact_Model->query_contact($id);
       $data['page']="contact";    
       $data['content']= $query ; 
       $data['path']="<li>我要提問</li><li  class='break'>&#187;</li>";
...
public function updatecontact(){
     ...
     $id=$this->input->post('nid');
        ...
        $this->load->library('contactClass');
                $contactClass= new contactClass;
                $contactClass=$this->Contact_Model->query_contact($id)->row();
                
                $contactClass->replycontent=$this->input->post('replycontent');
                $contactClass->replytime= date('Y-m-d H:i:s');
                $contactClass->status="2";   
                
                $this->Contact_Model->update_contact($id,$contactClass);
                   $access_id=accessLog('U','Contact',$id,$this->session->userdata('userRealname').'回覆我要提問【'.$this->input->post('subject').'】','S');
     ...
     public function deleteRecord($pid){
           if($pid!='')
        $column = $this->Contact_Model->delete_contact($pid);
        redirect(base_url().'contact/index', 'refresh');
    }
}
