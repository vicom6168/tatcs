...
class Analysis extends CI_Controller {
	   ...
	   public function index()
    {
     ...
     if( $qYear!='' &&  $qMonth!=''){
        ...
        $ans1=$this->Analysis_Model->query_associateReport($qYear,$qMonth,$qYearEnd,$qMonthEnd,$h1);
     ...
     $data['h1']=$h1;
       ...
       public function indexDetail($i,$qYear,$qMonth,$qYearEnd,$qMonthEnd,$h1){
            $ans1="";
    ...
    // $h1=$this->input->post('patientHospital')==null?"":$this->input->post('patientHospital');
     if( $qYear!='' &&  $qMonth!=''){
        ...
        $ans1=$this->Analysis_Model->query_associateReportDetail($i,$qYear,$qMonth,$qYearEnd,$qMonthEnd,urldecode($h1));
...
public function complication(){
     ...
     if( $qYear!='' &&  $qMonth!=''){
         for($i=0;$i<15;$i++){
        $ans[$i]=$this->Analysis_Model->query_complication($qYear,$qMonth,$qYearEnd,$qMonthEnd,$i,$h1);
         }
...
public function casenumber(){
     ...
     $currenYear=Date('Y');
     $qYear=$this->input->post('qYear')==null?"2018":$this->input->post('qYear');
