   ...
   //  $sql.= "  and  (createTime between subdate(current_date(), 1) and current_date()) and  (createTime between subdate(current_date(), ".$days.") and current_date()) ";
    ...
    function export_patientlistCVS($d1,$d2,$h,$t1,$t2,$t3,$v){
        ...
        //$sql.=", patientSurgeon,patientSurgeon2,patientSurgeon3,patientSurgeon4 ";
         $sql.=", if((select case when isExport='1' then true else false end from user t2 where t2.userRealname=t1.patientSurgeon),t1.patientSurgeon,if(t1.patientSurgeon<>'','不匯出',''))  as 'patientSurgeon 1'";
       $sql.=", if((select case when isExport='1' then true else false end from user t2 where t2.userRealname=t1.patientSurgeon2),t1.patientSurgeon2,if(t1.patientSurgeon2<>'','不匯出',''))    as 'patientSurgeon 2'";
       $sql.=", if((select case when isExport='1' then true else false end from user t2 where t2.userRealname=t1.patientSurgeon3),t1.patientSurgeon3,if(t1.patientSurgeon3<>'','不匯出',''))  as 'patientSurgeon 3'";
       $sql.=", if((select case when isExport='1' then true else false end from user t2 where t2.userRealname=t1.patientSurgeon4),t1.patientSurgeon4,if(t1.patientSurgeon4<>'','不匯出',''))  as 'patientSurgeon 4'";
